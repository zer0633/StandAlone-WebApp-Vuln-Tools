...
white-host "if the service is not vulnerable"  -ForegroundColor yellow
...
write-host "checking for 'SeImpersonatePrivilege'"  -ForegroundColor green
$value = whoami /priv | findstr /B /C:"SeImpersonatePrivilege"
if ($value -eq'SeImpersonatePrivilege')
{
    write-host "possibly vulnerable to juicy potato" -ForegroundColor Red -BackgroundColor Black
}
else{
 write-host "Target is not vulnerable to juicy potato" -ForegroundColor yellow
}
...
write-host "checking for 'Always Intall Elevated Privesc vector'"  -ForegroundColor green
...
foreach ($name in $i)
{c:\users\user\Desktop\accesschk.exe /accepteula -uwcqv $env:USERNAME  $name |findstr "R" | findstr /v "Accesschk v6.15 - Reports effective permissions for securable objects 
Copyright (C) 2006-2022 Mark Russinovich"  | findstr /v "SERVICE_ALL_ACCESS" | findstr /v "SERVICE_QUERY_STATUS" | findstr /v "SERVICE_QUERY_CONFIG" | findstr /v "SERVICE_INTERROGATE" | findstr /v "SERVICE_ENUMERATE_DEPENDENTS" | findstr /v "SERVICE_START" | findstr /v "READ_CONTROL" | findstr /v "SERVICE_PAUSE_CONTINUE" | findstr /v "SERVICE_STOP" | findstr /v "SERVICE_USER_DEFINED_CONTROL" |  findstr /v "WRITE_DAC" | findstr /v "OWNER" | select-string -pattern SERVICE_CHANGE_CONFIG -CONTEXT 1,0}


if ("SERVICE_CHANGE_CONFIG" -eq $null)
{
...
else{write-host "NOT VULNERABLE" -ForegroundColor Yellow} 
...
write-host "copy C:\Users\$env:username\reverse.exe " "c:\binary path" "/Y " -ForegroundColor green
$z = get-service
foreach ($val in $z)
...
cmd.exe /c wmic service get pathname | findstr  "C:\\Program Files\\" |findstr  /v """"
